...
| A failed artifact fetch never records a credential from the URL | `artifact.safeURL` keeps the scheme, host and path and drops userinfo, query and fragment; `artifact.transportCause` strips the `*url.Error` net/http wraps a transport failure in, because its `Error()` embeds the RAW url verbatim. **Both halves are needed and the first fix here shipped only one:** `url.URL.Redacted()` replaces the password and nothing else, so a username-only credential and the whole signed query string survive it — and even with our own half redacted, the standard library had already written the full URL into the other half. These errors are deliberately propagated: the publish path logs them and `artifact.Reason` turns them into a review note, and `audit_log` is append-only by trigger and can never be redacted afterwards. `TestFetch_AFailedFetch_NeverEchoesACredentialFromTheURL` covers a password, a username-only token, an `X-Amz-Signature` query, an unnamed token parameter and a fragment, and asserts the host still survives — a diagnosis with no host in it is not a diagnosis |
