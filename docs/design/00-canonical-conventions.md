...
There is exactly one source: `internal/authz/catalogue.go`. It generates the OpenAPI
`x-regserve-permission` metadata, the PAT scope enum, and
[`docs/reference/permissions.md`](../reference/permissions.md) — which is written by `make
gen-authz` and regenerated by gate `GEN001`, so a hand edit is caught rather than reviewed for. A
hand-written permission list anywhere else is forbidden.
...
- **Every key is written as a whole quoted literal.** Gate `AUTHZ001` reads the file as text and
...
The guarding itself lives in `internal/identity/guard`, which builds the one client everything goes
through — including, from Phase 3, the artifact fetcher. It sits under `internal/identity/` rather
than at the top of the tree because `NET001` is written against the tree: widening the gate to make
room for the package that implements it would be a strange way to keep it.
